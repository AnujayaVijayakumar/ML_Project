...
class CSVDataWriter:
    def __init__(self, path="process_video/keypoint_mapping.yml"):
        self.frame_list = []
        self.timestamps = []
        self.column_names = self.load_keypoint_mapping_from_file(path)
    ...
    def get_frames(self):
        return pd.DataFrame(self.frame_list, columns=self.column_names, index=self.timestamps)